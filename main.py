...
def collect_player_stats(league_id, year):
    ...
    for name, s in players.items():
        mp = s["matches"]
        try:
            row = {"player_name": name, "matches_played": mp, "goals_scored": s["goals"], "errors_leading_to_goal": s["errors"], "yellow_cards_avg": round(s["yc"] / mp, 1), "updated_at": datetime.now(timezone.utc).isoformat()}
