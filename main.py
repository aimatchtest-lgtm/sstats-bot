...
CURRENT_YEAR = 2026
SEASONS_TO_CHECK = [CURRENT_YEAR, CURRENT_YEAR - 1] # Только текущий и прошлый сезон
    ...
    for attempt in range(retries):
        try:
            ...
            if r.status_code == 200:
                ...
                time.sleep(10) # Лимит 150 req/min, спим 10 сек при ошибке
...
def get_glicko(game_id):
    ...
    if not data: return {}
    return {"home_rating": data.get("homeRating"), "away_rating": data.get("awayRating")}

def get_h2h_stats(team1_id, team2_id, year):
    if not team1_id or not team2_id:
        return {"matches_count": 0, "games": [], "avg_total_goals": 0, "avg_yellow_cards": 0}
    
    # Используем параметр bothTeams для получения H2H напрямую от API, если он поддерживается, 
    # иначе используем старый метод фильтрации списка
    games = safe_get(f"{BASE}/games/list?teamid={team1_id}&year={year}&limit=50")
    if not games: return {"matches_count": 0, "games": [], "avg_total_goals": 0, "avg_yellow_cards": 0}
    
    h2h = {"total_goals": [], "yellow_cards": [], "matches_count": 0, "games": []}
    t2 = str(team2_id)
    found = 0
    
    for g in games:
        home_id_g = str(g.get("homeTeam", {}).get("id", ""))
        away_id_g = str(g.get("awayTeam", {}).get("id", ""))
        
        if home_id_g != t2 and away_id_g != t2:
            continue
            
        gid = g.get("id")
        if not gid: continue
        
        # Для H2H нам нужны только завершенные матчи
        if g.get("status") not in [8, 9, 10]:
            continue

        fd = safe_get(f"{BASE}/games/{gid}")
        if not fd: continue
        
        game_data = fd.get("game", {})
        stats_data = fd.get("statistics", {})
        
        goals = (game_data.get("homeFTResult") or 0) + (game_data.get("awayFTResult") or 0)
        yc = (stats_data.get("yellowCardsHome") or 0) + (stats_data.get("yellowCardsAway") or 0)
        ...
        found += 1
        if found >= 5: break
        time.sleep(0.1) # Небольшая пауза внутри H2H, чтобы не спамить API

    result = {"matches_count": h2h["matches_count"], "games": h2h["games"]}
    for key in ["total_goals", "yellow_cards"]:
        values = h2h[key]
...
def generate_verdicts(game, statistics, odds_list, referee_name, home_id, away_id, home_name, away_name, year):
    ...
    if "total_goals" in lines:
        ...
        # Корректировка на травмы и Glicko
        if glicko:
            diff_rating = (glicko.get("home_rating", 1500) or 1500) - (glicko.get("away_rating", 1500) or 1500)
            if diff_rating > 200: pred += 0.2 # Хозяева сильно сильнее
            elif diff_rating < -200: pred -= 0.2
        ...
        verdicts.append({
            ...
            "analysis_json": {
                ...
                "injuries_note": f"Травмы: {len(injuries)}" if injuries else "Без ключевых травм"
            }
        })
    ...
    # Аналогично можно добавить CORNERS и FOULS...
    return verdicts
...
def update_team_stats_incremental(home_id, home_name, away_id, away_name, stats, year, league_id, league_name):
    ...
    for tid, tname, is_home in [(home_id, home_name, True), (away_id, away_name, False)]:
        try:
            ...
            xg_for = stats.get("calculatedXgHome") or 0 if is_home else stats.get("calculatedXgAway") or 0
            xg_against = stats.get("calculatedXgAway") or 0 if is_home else stats.get("calculatedXgHome") or 0
            yc = stats.get("yellowCardsHome") or 0 if is_home else stats.get("yellowCardsAway") or 0
            corners = stats.get("cornerKicksHome") or 0 if is_home else stats.get("cornerKicksAway") or 0
            fouls = stats.get("foulsHome") or 0 if is_home else stats.get("foulsAway") or 0
            ...
            row = {
                ...
                "xg_for_avg": round((current.get("xg_for", 0) * (matches_played-1) + xg_for) / matches_played, 2) if matches_played > 1 else xg_for,
                "avg_yellow_cards_for": round((current.get("avg_yellow_cards_for", 0) * (matches_played-1) + yc) / matches_played, 2) if matches_played > 1 else yc,
                "avg_corners_for": round((current.get("avg_corners_for", 0) * (matches_played-1) + corners) / matches_played, 2) if matches_played > 1 else corners,
                "avg_fouls_for": round((current.get("avg_fouls_for", 0) * (matches_played-1) + fouls) / matches_played, 2) if matches_played > 1 else fouls,
...
def process_league_season(league_id, league_name, year):
    """Обрабатывает один сезон одной лиги"""
    ...
    # Получаем ID уже известных матчей, чтобы не дублировать
    ...
    for gs in games_list:
        ...
        # Скачиваем полные данные матча ОДИН РАЗ
        ...
        if status == "scheduled" and mt and mt <= datetime.now(timezone.utc) + timedelta(days=3):
            ...
            if total_verdicts_saved > 0:
                print(f"    ✅ {home.get('name')} vs {away.get('name')}: {total_verdicts_saved} вердиктов")

        time.sleep(0.2) # Пауза между матчами, чтобы не превысить 150 req/min
